...
function checkInvalidMappingColumn({ generatedLine, generatedColumn, line, eol }) {
  const maxColumnIndex = line.length - 1; // Columns are 0-based
...
function computeFileSizes(sourceMapData) {
  const { consumer, codeFileContent } = sourceMapData;
  const eol = detectEOL(codeFileContent); // Assume only one EOL is used
